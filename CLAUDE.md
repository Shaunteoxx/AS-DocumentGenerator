...
- `POST /documents/{id}/regenerate-section` — regenerates one section and returns it as a **preview only** (no Drive write). Response includes `updated_section`, `full_content`, and `next_heading` for the subsequent save. The user reviews the proposed change (with an optional compare-with-current view) before anything is persisted.
- `POST /documents/{id}/save-section` — called once the user **confirms** the previewed change; writes it back **in place** via `_replace_doc_section()` (Google Docs API `batchUpdate`): locates the section by heading text, deletes only that range, and re-inserts the new content with heading styles, bullets, and bold re-applied. Every other section keeps its original formatting. Falls back to a full-document re-upload (`_update_drive_doc`) only if the section heading can't be located. (Splitting regenerate from save is what lets the user review before the AI edit lands in Drive.)
