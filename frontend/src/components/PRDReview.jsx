...
export default function PRDReview({ prd, onConfirm }) {
  const [fullPrd, setFullPrd] = useState(prd)
  const [sectionStates, setSectionStates] = useState({})

  const sections = parseSections(fullPrd)
  ...
  const handleRegenerate = async (heading) => {
    ...
    try {
      const res = await authFetch(`${API}/prd/regenerate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ crd: fullPrd, section: heading, instruction: state.instruction }),
      })
            ...
            <div key={section.heading} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="px-6 py-3 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-orange-700 bg-white border border-orange-200 rounded-lg hover:bg-orange-50 transition-colors"
                >
              ...
              {state.open && (
                <div className="px-6 pb-5 pt-3 bg-orange-50/60 border-t border-orange-100 space-y-3">
                  ...
                  <textarea
                    ...
                    placeholder="e.g. Add more detail to the feature requirements, clarify the acceptance criteria…"
                    rows={3}
                    className="w-full px-3 py-2.5 text-sm bg-white border border-orange-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent placeholder:text-gray-400 resize-none"
                  />
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-4 py-1.5 text-sm font-medium text-white bg-orange-500 rounded-lg hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    >
                      <SparklesIcon className="w-3.5 h-3.5" />
